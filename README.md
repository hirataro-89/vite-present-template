...
`src/public/images/` 内の以下のファイルを自分のものに差し替えます。

| ファイル | 用途 | 推奨サイズ・形式 |
|---|---|---|
| `ogp.png` | SNSシェア時のサムネイル | 1200×630 PNG / JPG（**SVGはX/Slackなど主要SNSで非対応**） |
| `favicon.svg` | モダンブラウザ用のfavicon | 64×64程度のSVG |
| `favicon.png` | 古いブラウザ用フォールバック | 32×32 PNG |
| `apple-touch-icon.png` | iOSのホーム画面追加時アイコン | 180×180 PNG |
