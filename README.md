...
│   └── scss/
│       ├── base/        # リセットCSS、デザイントークン（CSS変数）、ベースとなるタグのスタイル
...
[`src/scss/style.scss`](src/scss/style.scss) で各ファイルを明示的に `@use` しています。
...
読み込み順序が意味を持つため（例: `layout/container` は `layout/header` より先に記述）、ファイルを追加する場合はこのリストに明示的に追加してください。

## デザイントークン（CSS変数）について

配色・フォント・余白などの共通値は、[`src/scss/base/_variables.scss`](src/scss/base/_variables.scss) に **CSS カスタムプロパティ（CSS変数）** としてまとめています。
