...
```
src/scss/
├── style.scss               ← @use でまとめるだけ（順序に意味あり）
│
├── base/                    ← グローバル基盤
...
│
└── components/              ← 単独で使う再利用パーツ
    ├── _button.scss         ← .button / .button-outline
    └── _section-title.scss  ← .section-title
```

`style.scss` の `@use` 順序は変更しないこと。`layout/container` が `layout/header` より先にある理由は、`@scope (.header)` 内の `.container` 追加スタイルが後勝ちになるよう保証するため。
